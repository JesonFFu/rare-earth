...
def build_model(client, name: str = "sin980_qbic_2d"):
    ...
    create_rectangle(geom, "re_top", ["-fill*P/2", y_air], ["fill*P", "h_re"], "Rare-earth conformal top layer")
    create_rectangle(geom, "re_side_l", ["-fill*P/2-h_re", y_sin], ["h_re", "h_sin"], "Rare-earth left sidewall layer")
    create_rectangle(geom, "re_side_r", ["fill*P/2", y_sin], ["h_re", "h_sin"], "Rare-earth right sidewall layer")
    create_rectangle(geom, "re_bottom_l", ["-P/2", y_sin], ["(1-fill)*P/2-h_re", "h_re"], "Rare-earth left groove bottom layer")
    create_rectangle(geom, "re_bottom_r", ["fill*P/2+h_re", y_sin], ["(1-fill)*P/2-h_re", "h_re"], "Rare-earth right groove bottom layer")
    create_rectangle(geom, "air_slot_l", ["-P/2", "h_pml+h_si_buf+h_sio2+h_re"], ["(1-fill)*P/2-h_re", "h_sin-h_re"], "Left remaining air groove")
    create_rectangle(geom, "air_slot_r", ["fill*P/2+h_re", "h_pml+h_si_buf+h_sio2+h_re"], ["(1-fill)*P/2-h_re", "h_sin-h_re"], "Right remaining air groove")
    create_rectangle(geom, "air_cap_l", ["-P/2", y_air], ["(1-fill)*P/2", "h_re"], "Left air cap over groove")
    create_rectangle(geom, "air_cap_r", ["fill*P/2", y_air], ["(1-fill)*P/2", "h_re"], "Right air cap over groove")
...
def mode_result(period_nm: float, fill: float, kx_norm: float, idx: int, freq_thz: complex) -> ModeResult:
    ...
    gap_nm = period_nm * (1.0 - fill)
    remaining_air_gap_nm = gap_nm - 32.0
    score = wl_error + 10.0 * q_penalty
    return ModeResult(period_nm, fill, gap_nm, remaining_air_gap_nm, kx_norm, idx, freq_thz.real, freq_thz.imag, wavelength_nm, q, score)
...
def write_csv(path: Path, rows: list[ModeResult]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(["period_nm", "fill", "gap_nm", "remaining_air_gap_nm", "kx_norm", "mode_index", "freq_thz_real", "freq_thz_imag", "wavelength_nm", "q_value", "score"])
        for r in rows:
            writer.writerow([r.period_nm, r.fill, r.gap_nm, r.remaining_air_gap_nm, r.kx_norm, r.mode_index, r.freq_thz_real, r.freq_thz_imag, r.wavelength_nm, r.q_value, r.score])
        ...
        for row in csv.DictReader(handle):
            rows.append(
                ModeResult(
                    float(row["period_nm"]),
                    float(row["fill"]),
                    float(row.get("gap_nm", float(row["period_nm"]) * (1.0 - float(row["fill"])))),
                    float(row.get("remaining_air_gap_nm", float(row["period_nm"]) * (1.0 - float(row["fill"])) - 32.0)),
                    float(row["kx_norm"]),
                    int(row["mode_index"]),
